...
/* ---------------------------------------------------------------------- *
 * PWM3 runs in TRUE INDEPENDENT OUTPUT mode (PMOD = 0b11) with the
 * dead-time generator disabled (DTC = 0b10).
 *
 * That generator was the source of the 10 ns blip on GH.  The artifact
 * tracked CLAMP_DT_NS exactly - 100 ns gave a blip 100 ns after the override
 * handoff, 500 ns moved it to 500 ns - and setting DTC = 0b10 removed it
 * entirely.  Complementary mode cannot avoid it, because the dead-time stage
 * is unavoidably in the path and re-triggers on every live reconfiguration.
 *
 * Independent mode has no dead-time stage at all.  The four 100 ns gaps are
 * therefore placed explicitly instead of being generated by DTR3/ALTDTR3:
 *
 *     GH pin (PWM3H), active high : starts at PHASE3,  width PDC3
 *     GL pin (PWM3L), active LOW  : starts at SPHASE3, width SDC3
 *
 * POLL = 1 makes PWM3L active-low, so a duty of zero parks GL HIGH while a
 * duty of zero parks GH LOW - the correct idle state, reached with no edges
 * at all.  SWAP is no longer needed: both edges are positioned directly.
 * ---------------------------------------------------------------------- */
#define CLAMP_DUTY_OFF 0u

/* PHASE3/SPHASE3 are phase shifts against the master time base.  The data
 * sheet gives the range but never the direction, so this converts a desired
 * START OFFSET into a register value both ways.  Both registers use the same
 * convention, so one flag flips them together.
 *
 * SCOPE CHECK: the clamp pulse must sit inside PWM1L's long high.  If it comes
 * out mirrored about the period, set this to 0. */
#define CLAMP_PHASE_ADVANCES 1

#if CLAMP_PHASE_ADVANCES
#define CLAMP_SHIFT(start, per) ((uint16_t)((per) - (start)))
#else
#define CLAMP_SHIFT(start, per) ((uint16_t)(start))
#endif
...
static uint16_t rd_phase3, rd_pdc3;   /* GH: start, width */
static uint16_t rd_sphase3, rd_sdc3;  /* GL: start, width (active low) */
...
_PWMSpEventMatchInterrupt(void)
{
    ...
    case 0:
        ...
        {
            rdson_pending = 0;
            /* Both outputs' geometry, loaded while the override masks the
             * pins and both duties are still zero.  With duty = 0 the
             * generator produces no edges, so changing the phase registers
             * here - they are not double-buffered - cannot disturb anything. */
            PHASE3 = rd_phase3;   /* GH start  */
            PDC3 = rd_pdc3;       /* GH width  */
            SPHASE3 = rd_sphase3; /* GL start  */
            SDC3 = rd_sdc3;       /* GL width  */
            rdson_state = 1;
        }
        break;
...
void Rdson_Precompute(uint32_t freq, uint8_t duty)
{
     ...
     *
     * In Independent mode each output is just "go active at my phase offset,
     * stay active for my duty", so all four edges are stated directly.  No
     * dead-time register is involved anywhere in this - that is the point.
     *
     * PWM1L rises at t1r and falls at the rollover t1f.  ALTDTR1 is already
     * loaded by PWM_Mode2() before this runs, so the turn-on gaps hold
     * whatever dead time PWM1 itself is using.
     * ------------------------------------------------------------------ */
    {
        uint16_t d = CLAMP_DT_CNT;
        uint16_t t1r = (uint16_t)(rd_slow_duty + ALTDTR1);
        uint16_t t1f = rd_slow_per;

        /* GL is active-low: its "active" window is when GH is allowed on,
         * opened one gap after PWM1L rises and closed one gap before it
         * falls. */
        rd_sphase3 = CLAMP_SHIFT((uint16_t)(t1r + d), rd_slow_per);
        rd_sdc3 = (uint16_t)(t1f - t1r - (2u * d));

        /* GH sits one further gap inside GL at both ends. */
        rd_phase3 = CLAMP_SHIFT((uint16_t)(t1r + (2u * d)), rd_slow_per);
        rd_pdc3 = (uint16_t)(t1f - t1r - (4u * d));
    }
}
...
{
    /* ------------------------------------------------------------------ *
     * True Independent Output mode.  Each pin is driven by its own pair of
     * registers, with no complementary coupling between them:
     *
     *     GH pin (PWM3H), active high : starts at PHASE3,  width PDC3
     *     GL pin (PWM3L), active LOW  : starts at SPHASE3, width SDC3
     *
     * Nothing here depends on the dead-time generator, which is the block
     * that was emitting the 10 ns blip on GH.
     * ------------------------------------------------------------------ */
    /* PMOD must be set before PTEN = 1 (data sheet Register 15-20, Note 1).
     * PWM_Mode2() calls this while the module is disabled, which is why the
     * mode change is legal here and nowhere else. */
    IOCON3bits.PENH = 1;
    IOCON3bits.PENL = 1;
    IOCON3bits.PMOD = 0b11; /* True Independent - no dead-time stage      */
    IOCON3bits.POLH = 0;    /* GH active high: duty 0 parks it LOW        */
    ...
    PWMCON3bits.MDCS = 0; /* PDC3/SDC3 are the duty sources, not MDC    */
    PWMCON3bits.IUE = 0;  /* duty latches at the period boundary        */

    /* The whole reason for this rewrite.  DTR3/ALTDTR3 are now unused and are
     * left at zero so nothing can re-trigger them. */
    ...
    /* Parked: no phase, no duty, therefore no edges at all. */
    PHASE3 = 0;
    SPHASE3 = 0;
    PDC3 = CLAMP_DUTY_OFF;
    SDC3 = CLAMP_DUTY_OFF;
